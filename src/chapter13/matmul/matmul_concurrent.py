...
def matrix_multiply(matrix_a: Matrix, matrix_b: Matrix) -> Matrix:
    """Multiply two matrices, calculating each row of the solution concurrently."""
...
def process_column(args: tuple) -> Column:
    """Creates 1 column of the solution matrix"""
...
if __name__ == "__main__":
    cols = 4
    rows = 2
    matrix_a = [[random.randint(0, 10) for _ in range(cols)] for _ in range(rows)]
    print(f"Matrix A: {matrix_a}")
    matrix_b = [[random.randint(0, 10) for _ in range(rows)] for _ in range(cols)]
