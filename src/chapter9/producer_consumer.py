...
class Producer(Thread):
    ...
    def __init__(self, name: str, maximum_items: int = 5) -> None:
        super().__init__()
        self.counter = 0
        self.name = name
    ...
    def run(self) -> None:
        ...
        while self.counter < self.maximum_items:
            # wait until the buffer have some empty slots
            empty.acquire()
...
class Consumer(Thread):
    ...
    def __init__(self, name: str, maximum_items: int = 10) -> None:
        super().__init__()
        self.name = name
        self.idx = 0
        self.counter = 0
    ...
    def run(self) -> None:
        while self.counter < self.maximum_items:
            # wait until the buffer has some new items to consume
            full.acquire()
            # critical section for changing the buffer
            mutex.acquire()
