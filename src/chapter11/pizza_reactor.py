...
Data = bytes
Action = T.Union[T.Callable[[socket], None], T.Tuple[T.Callable[[socket, Data], None], str]]
Mask = int
...
class EventLoop:
    ...
    def register_event(self, source: socket, event: Mask, action: Action) -> None:
        """Register the given socket for the given event type
        with the given to be executed the event occurs."""
        key = source.fileno()
    ...
    def run_forever(self) -> None:
        ...
        while True:
            ...
            for reader in readers:
                source, _, action = self.readers.pop(reader)
                action(source)

            for writer in writers:
                source, _, action = self.writers.pop(writer)
                action, msg = action
                action(source, msg)
