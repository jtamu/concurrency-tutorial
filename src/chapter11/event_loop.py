...
class Event:
    ...
    def execute_action(self) -> None:
        """Execute the action of the event and register the next event if exists"""
        self._action(self)
        if self._next_event:
...
class EventLoop:
    """A class representing an event loop that maintains a deque of Events and executes them."""
    ...
    def run_forever(self) -> None:
        """Execute the actions of the events in the deque in a loop until the program is terminated."""
...
def knock(event: Event) -> None:
    """A callback function that prints the name of the event and sleeps for one second."""
    print(event.name)
    sleep(1)
